...
import { Plus, Search, Pin, Trash2, Sparkles, Save, X, FileText } from 'lucide-react';
...
export function NotesView({ profile, initialNotes, initialFolders }: any) {
  ...
  return (
    <div style={{ display: 'flex', height: '100dvh', overflow: 'hidden' }}>
      {/* Sidebar */}
      <div style={{ width: '260px', borderRight: '1px solid hsl(240 6% 14%)', display: 'flex', flexDirection: 'column', background: 'hsl(240 10% 4%)' }}>
        <div style={{ padding: '12px', borderBottom: '1px solid hsl(240 6% 14%)', display: 'flex', gap: '8px' }}>
          ...
          <button onClick={newNote} style={{ width: '34px', height: '34px', borderRadius: '8px', background: 'hsl(205 90% 48% / 0.15)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><Plus size={15} color="hsl(205, 90%, 60%)" /></button>
        </div>
        <div style={{ flex: 1, overflowY: 'auto', padding: '6px' }}>
          ...
          {filtered.length === 0 && <div className="empty" style={{ padding: '32px 16px' }}><FileText size={28} color="hsl(240 5% 35%)" /><p style={{ fontSize: '13px', color: 'hsl(240 5% 50%)' }}>{search ? 'No results' : 'No notes yet'}</p>{!search && <button onClick={newNote} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'hsl(205, 90%, 60%)', fontSize: '12px' }}>Create one</button>}</div>}
        </div>
      </div>
        ...
        {active ? (
          <>
            <div style={{ padding: '10px 16px', borderBottom: '1px solid hsl(240 6% 14%)', display: 'flex', alignItems: 'center', gap: '10px' }}>
              <input value={title} onChange={e => setTitle(e.target.value)} onBlur={() => save(active.id, title, content)} style={{ flex: 1, background: 'transparent', border: 'none', fontSize: '16px', fontWeight: 600, outline: 'none', padding: 0, color: 'hsl(0 0% 90%)' }} placeholder="Note title" />
              <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                {saving && <span style={{ fontSize: '11px', color: 'hsl(240 5% 50%)' }}>Saving…</span>}
                <button onClick={summarize} disabled={summarizing} style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '6px 12px', borderRadius: '8px', background: 'hsl(205 90% 48% / 0.1)', border: '1px solid hsl(205 90% 48% / 0.2)', color: 'hsl(205, 90%, 60%)', fontSize: '12px', cursor: 'pointer', fontWeight: 500 }}>
                  <Sparkles size={12} />{summarizing ? 'Summarizing…' : 'AI Summary'}
                </button>
                <button onClick={() => save(active.id, title, content)} style={{ padding: '6px', borderRadius: '8px', background: 'none', border: 'none', cursor: 'pointer', color: 'hsl(240 5% 55%)', display: 'flex' }}><Save size={15} /></button>
                <button onClick={() => del(active.id)} style={{ padding: '6px', borderRadius: '8px', background: 'none', border: 'none', cursor: 'pointer', color: 'hsl(240 5% 55%)', display: 'flex' }}><Trash2 size={15} /></button>
              </div>
            </div>
            {active.ai_summary && (
              <div style={{ margin: '12px 16px 0', padding: '10px 14px', borderRadius: '10px', background: 'hsl(205 90% 48% / 0.06)', border: '1px solid hsl(205 90% 48% / 0.15)' }}>
            ...
            <div style={{ padding: '8px 16px', borderTop: '1px solid hsl(240 6% 14%)', display: 'flex', gap: '12px', fontSize: '11px', color: 'hsl(240 5% 45%)' }}>
          ...
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '16px', padding: '40px' }}>
            ...
            <button onClick={newNote} className="btn btn-primary"><Plus size={15} /> New Note</button>
          </div>
        )}
      </div>
    </div>
...
function NoteItem({ note, active, onSelect, onPin, onDelete }: any) {
  return (
    <div onClick={onSelect} style={{ padding: '9px 10px', borderRadius: '8px', cursor: 'pointer', background: active ? 'hsl(205 90% 48% / 0.1)' : 'transparent', transition: 'background 0.15s', marginBottom: '2px' }} className="note-item">
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '4px' }}>
        ...
        <div style={{ display: 'flex', gap: '2px', opacity: 0 }} className="note-actions">
          <button onClick={onPin} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '2px', color: note.is_pinned ? 'hsl(205, 90%, 60%)' : 'hsl(240 5% 50%)', display: 'flex' }}><Pin size={10} /></button>
          <button onClick={(e) => { e.stopPropagation(); onDelete(); }} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '2px', color: 'hsl(240 5% 50%)', display: 'flex' }}><Trash2 size={10} /></button>
        </div>
      </div>
